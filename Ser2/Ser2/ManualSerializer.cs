...
namespace Ser2.Serializer
{
    public class ManualSerializer
    {
        ...
        public static SByte ReturnSByteValueFromByteArray(int StartIndexInBytes, int VarSizeInBytes, Byte[] arraytoParse)
        {
            SByte temp = 0;
            //if (File.Exists(fileName))
            //{
                ...
                //{
                    ...
                    {
                        temp = ((sbyte)(temp << 4));
                        temp = ((sbyte)(temp | Convert.ToSByte(Convert.ToChar(arraytoParse[position]).ToString(), 16)));
                        position++;
        ...
        //Reads string saved to file (const at constructor), it returns the value of short in Hex as string
        //Big Endianess does not work. 0d9417 --> 0xc924
        public static short ReturnInt16ValueFromByteArray(int StartIndexInBytes, int VarSizeInBytes, bool BigEndianess, Byte[] arraytoParse)
        {
            short temp = 0;
            short tempread = 0;
            //if (File.Exists(fileName))
            //{
                ...
                //{
                    ...
                    if (BigEndianess)
                    {
                        ...
                        {
                            temp = (short)(temp << 4);
                            temp = ((short)(temp | (short)Convert.ToByte(Convert.ToChar(arraytoParse[position]).ToString(), 16)));
                            position++;
                        }
                    }
                    if (!BigEndianess)
                    {
                        ...
                        {
                            temp = (short)(temp << 4);
                            tempread = (short)Convert.ToByte(Convert.ToChar(arraytoParse[position - 1]).ToString(), 16);
                            temp = ((short)(temp | tempread));
                            temp = (short)(temp << 4);
                            tempread = (short)Convert.ToByte(Convert.ToChar(arraytoParse[position]).ToString(), 16);
                            temp = ((short)(temp | tempread));
                            position = position - 2;
                        }
                    }
        ...
        //Reads string saved to file (const at constructor), it returns the value of integer in Hex as string
        //Big Endianess does not work.
        public static UInt32 ReturnUint32ValueFromByteArray(int StartIndexInBytes, int VarSizeInBytes, bool BigEndianess, Byte[] arraytoParse)
        {
            UInt32 temp = 0;
            UInt32 tempread = 0;
            //if (File.Exists(fileName))
            //{
                ...
                //{
                    ...
                    if (BigEndianess)
                    {
                        ...
                        {
                            temp = (uint)(temp << 4);
                            temp = ((uint)(temp | (uint)Convert.ToByte(Convert.ToChar(arraytoParse[position]).ToString(), 16)));
                            position++;
                        }
                    }
                    if (!BigEndianess)
                    {
                        ...
                        {
                            temp = (uint)(temp << 4);
                            tempread = (uint)Convert.ToByte(Convert.ToChar(arraytoParse[position - 1]).ToString(), 16);
                            temp = ((uint)(temp | tempread));
                            temp = (uint)(temp << 4);
                            tempread = (uint)Convert.ToByte(Convert.ToChar(arraytoParse[position]).ToString(), 16);
                            temp = ((uint)(temp | tempread));
                            position = position - 2;
                        }
                    }
